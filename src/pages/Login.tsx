...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) { toast.error('Please fill in all fields'); generateCaptcha(); return; }
    if (userCaptcha !== captcha.answer) { toast.error('Incorrect captcha answer'); generateCaptcha(); return; }
    setIsLoading(true);
    try {
      ...
      if (error) {
        ...
        toast.success('Access Granted - Welcome to Lab Monitoring System');
        navigate('/dashboard');
      }
    } catch {
  ...
  return (
    <div className="h-screen w-full flex font-sans overflow-hidden bg-white">
      ...
      {/* ── LEFT PANEL: CM Logo as full cover ── */}
      <div className="hidden lg:flex flex-col items-end justify-end w-1/2 xl:w-[58%] h-full relative overflow-hidden">
        {/* Pakistan flag stripe at top & bottom */}
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-green-700 via-yellow-500 to-green-700 z-10" />
        <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-gradient-to-r from-green-700 via-yellow-500 to-green-700 z-10" />

        {/* Full bleed cover image */}
        <div
          className="absolute inset-0 animate-in fade-in duration-700"
          style={{
            backgroundImage: "url('/CM_Logo.jpg')",
      ...
      <div className="flex-1 flex items-center justify-center bg-white px-6 py-10 relative">
        ...
        <div className="w-full max-w-[360px] animate-in fade-in zoom-in-95 duration-700 relative z-10">
          <div className="relative p-[3px] rounded-[3rem] overflow-hidden shadow-[0_0_60px_rgba(0,0,0,0.1)] hover:shadow-[0_0_80px_rgba(245,158,11,0.15)] transition-shadow duration-500">
            ...
            <div className="relative w-full space-y-6 px-8 py-10 rounded-[calc(3rem-3px)] bg-white">
              ...
              <div className="text-center space-y-3 pt-2">
                <h1 className="text-2xl font-black tracking-[0.1em] uppercase text-slate-900">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                ...
                {/* CAPTCHA */}
                <div className="space-y-3 p-4 rounded-[2rem] bg-slate-50 border border-slate-200">
                  <div className="flex items-center justify-between">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                      Verification Required
                    </Label>
                    <button type="button" onClick={generateCaptcha} className="text-slate-300 hover:text-[#F59E0B] transition-all">
